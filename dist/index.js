...
function usePhoneNumber(source = phoneNumberData) {
    ...
    const localPhone = ref(localPhoneDigitsForCountry(countries.value, selectedCountryKey.value, sourceValue.phone ?? ''));
    ...
    const selectedPhoneLengths = computed(() => phoneLengthsForCountry(countries.value, selectedCountryKey.value));
    const callingCode = computed(() => callingCodeForCountry(countries.value, selectedCountryKey.value));
    const mask = computed(() => phoneMaskForCountry(countries.value, selectedCountryKey.value));
    const isValid = computed(() => localPhone.value.length > 0 &&
        selectedPhoneLengths.value.includes(localPhone.value.length));
    const updatePhone = (value) => {
        const eventTarget = targetFromInput(value);
        const nextPhone = localPhoneDigitsForCountry(countries.value, selectedCountryKey.value, valueFromInput(value));
        const nextIsValid = nextPhone.length > 0 && selectedPhoneLengths.value.includes(nextPhone.length);
